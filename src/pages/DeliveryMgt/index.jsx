  ...
  return (
    <AppLayoutNew noHeader={true}>
      <div className="mx-4 sm:mx-7 my-10">
        <PageHeader children={'Delivery Management'} />
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-5 mt-5">
          {summary.map((item, idx) => (
            <div key={idx} className="bg-dimmed_white grid place-content-center text-center min-h-[120px] rounded-md">
          ...
          {tabs.map((tab, idx) => (
            <button
              ...
              className={`rounded-t-md min-w-[100px] px-5 py-1.5 font-medium opacity-80 ${
                activeTab === idx && 'border border-primary border-b-0 bg-white !text-primary'
              }`}
            >
              {tab}
            </button>
              ...
              <table className="min-w-[900px] w-full table-auto border-separate border-spacing-y-3 ">
                <thead className="">
                  <tr className="!text-left !opacity-70 !font-semibold">
                    <th className="text-sm py-3 border-y !font-semibold pl-3 w-[32%]s">User Name</th>
                    <th className="text-sm py-3 border-y !font-semibold">Email</th>
                    <th className="text-sm py-3 border-y !font-semibold"> Phone</th>
                    <th className="text-sm py-3 border-y !font-semibold"> User Role</th>
                    <th className="text-sm py-3 border-y !font-semibold"> Account Status</th>
                    <th className="text-sm py-3 border-y !font-semibold">Date Joined </th>
                  </tr>
                </thead>
                      ...
                      {data.data &&
                        data?.data?.map((item, idx) => (
                          ...
                          >
                            ...
                            <td className="text-sm py-2.5 pb-4">{item.user?.email}</td>
                            <td className="text-sm py-2.5 pb-4">{item.user?.phone}</td>
                            ...
                            <td className="text-sm  py-2.5 pb-4">{moment(item.createdDate).format('ll')}</td>
                          </tr>
                        ))}
                    </>
                  </tbody>
