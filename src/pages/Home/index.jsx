...
import { BsArrowDownLeft, BsArrowUpRight, BsEyeFill, BsEyeSlashFill, BsFillShieldLockFill } from 'react-icons/bs';
  ...
  const todaySummary = [
    ...
    },
    {
      ...
      desc:
        result?.salesVolume +
        ' sale(s), ' +
        percentageChange(summonthly?.monthlySales, summonthly?.previousMonthSales) +
        '% from last month',
    },
    {
      ...
      desc:
        result?.expenseVolume +
        ' expense(s), ' +
        percentageChange(summonthly?.monthlyExpense, summonthly?.previousMonthExpense) +
        '% from last month',
    },
    {
            ...
            <div className=" col-span-3 rounded min-h-[200px] bg-dimmed_white  p-5 !pb-7 ">
              ...
              <div className="border-y px-2 p-3.5  flex justify-between  sm:flex-row flex-col gap-5 mt-10">
                ...
                <CustomButton
                  ...
                  className={'!bg-[#eb57571a] !text-[#eb5757] border !border-[#eb5757] font-bold !py-2.5 rounded-lg'}
                >
                  Record Expenses
                </CustomButton>
              </div>
              <div className="md:flex grid grid-cols-2 gap-5 justify-between pt-10">
                <div>
                  ...
                  {accountLoading ? <Loading /> : <p className="font-semibold text-xl">{businessPoint} points</p>}
                  <p className="text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-xl">₦{summonthly?.businessDebt?.toFixed(2)}</p>
                  <p className="text-sm">
                ...
                <div>
                  ...
                  <p className="font-semibold text-xl">₦{summonthly?.customerDebt?.toFixed(2)}</p>
                  <p className="text-sm">Debt own by your customers.</p>
                  <CustomButton className={'!py-2 mt-3 !border-primary !text-white font-bold !rounded-lg'}>
                    Save
                  </CustomButton>
                </div>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-5 mx-4 sm:mx-7 mt-5">
            <div className="flex flex-col bg-dimmed_white min-h-[300px] rounded-md p-5">
              ...
              <div className="w-full border mt-auto rounded-md p-3  opacity-80 text-sm">
                <p>
                  💡 With Glowbiz coperative You have access to lorem ipsum dolor sit amet, consectetur adipisicing
                  elit. Dolores, laborum!
                </p>
              </div>
            </div>
            <div className="bg-dimmed_white min-h-[300px] rounded-md p-5">
              <div className="mb-6 flex justify-between items-center gap-5">
                <p className="font-medium text-lg opacity-80">Recent Deliveries</p>
                <button onClick={() => navigate('/delivery')} className="text-sm font-semibold flex items-center gap-2">
                  More <PiArrowRightThin />
                </button>
              </div>
              {[1, 2, 3].map((item) => (
                <div key={item} className="bg-[#493847]/5  p-3 mb-3 rounded-md text-xs flex justify-between">
                  <div className="flex gap-2 items-center">
                    ...
                    <div className="flex flex-col gap-1 ">
                      ...
                      <span className="-mt text-xs">To: Victoria Island, Lagos</span>
                      <span className={`text-xs capitalize font-medium text-[#1c5c4f]`}>completed</span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1 text-right">
                    <span className="font-medium opacity-80 text-sm ">{item * 2}7km</span>
                    <span className="font-medium opacity-80 text-sm ">₦200.00</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="px-4 sm:px-7 grid lg:grid-cols-3 gap-5">
              ...
              <div className="bg-white/90 rounded p-5 col-span-3 lg:col-span-1">
                <div className="flex justify-between items-center">
                  ...
                  <button className="text-primary text-sm font-medium opacity-80">VIEW ALL</button>
                </div>
                <div className="mt-5">
                  {lastFiveCredits.map((trnx, idx) => (
                    ...
                    >
                      <div className="flex gap-2 items-center">
                        <div
                          className={`grid place-content-center w-9 h-9 rounded-full ${
                            trnx.transaction_type == 'incoming' ? 'bg-[#bafbee]' : 'bg-[#fcd0cc]'
                          }`}
                        >
                          {trnx.transaction_type == 'incoming' ? <BsArrowDownLeft /> : <BsArrowUpRight />}
                        </div>
                        <div className="flex flex-col gap-[7px] ">
                          <span className="text-sm font-medium opacity-70">{trnx.name}</span>
                      ...
                      <div className="flex flex-col gap-[7px] text-right">
                        <span className="font-medium opacity-80 text-sm ">
                          {trnx.transaction_type == 'incoming' ? '+' : '-'} ₦{trnx.amount}
                        </span>
                        <span
                          className={`text-xs capitalize font-medium ${
                            trnx.status == 'successful' ? 'text-[#1c5c4f]' : 'text-[#cc746c]'
                          }`}
                        >
                          {trnx.status}
                        </span>
